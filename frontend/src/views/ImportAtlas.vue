<template>
  <div class="page">
    <div class="card">
      <h1>Importer Atlas Pro</h1>
      <p>Colle ici le JSON complet de Atlas Pro.</p>

      <textarea
        v-model="jsonText"
        placeholder="Collez ici le JSON Atlas..."
      ></textarea>

      <button @click="importerAtlas">
...
const importerAtlas = async () => {
  if (!jsonText.value) {
    message.value = "Collez le JSON Atlas d'abord.";
    return;
  }
  ...
  try {
    ...
    const res = await axios.post(
      "http://localhost:3000/api/import-atlas",
      data
    );
